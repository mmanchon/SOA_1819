...
void getAddr(int mode,Lba_info *info,FileSystem fileSystem){
    switch (mode){
        case 0:
            info->cluster_begin_lba = fileSystem.fat32.reservedSectors + (fileSystem.fat32.numberFat*fileSystem.fat32.sectorsPerFat);
            break;

        case 1://Moure per DS
            info->lba_adrr = (info->cluster_begin_lba + (info->i_cluster-2)*fileSystem.fat32.sectorsPerCluster)*fileSystem.fat32.sectorSize;
            break;
...
void goTroughFS(Lba_info info,char *argv, FileSystem fileSystem, Lba_info *trace, int *nTraces){
    ...
    if(info.dir.name[0] == 0){
        ...
        if(info.dir.isLfn == 0x0f){
            ...
            if (info.dir.name[6] != 0x7e && info.dir.name[6] != 0x20)
                printf("\n\nDir name: %s\n", info.dir.name);
